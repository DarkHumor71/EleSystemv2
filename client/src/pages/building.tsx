...
const Buildings: React.FC<BuildingsProps> = ({ head = true }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // setAuthToken(localStorage.token); // If you use a custom axios instance, set token here
        const response = await axios.get("/api/building");
        if (response.data) {
          const cleanedData = response.data.map(
            ({ _id, deleted_at, ...rest }: any) => rest
          );
          setRows(cleanedData);
