...
// Use ExpensesProps from types.d.ts
...
const Expenses: React.FC<ExpensesProps> = ({
  ...
  const [gcolumns, setGroupColumns] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [group, setGroupRows] = useState<any[]>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        if (single) {
          ...
          const cleanedData = Data.map((item: any, index: number) => {
            const {
              __v,
              _id,
              apartment,
          ...
          const cleanedGroupData = Object.values(groupedData).map(
            (entry: any) => ({
              ...entry,
...
const mapStateToProps = (state: any) => ({
  ...
  apartment_id: state.auth.apartment?._id || "",
});
